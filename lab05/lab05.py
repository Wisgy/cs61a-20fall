...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    tmp_location = make_city('temp', lat, lon)
    return get_name(city_a) if distance(city_a,tmp_location) < distance(city_b,tmp_location) else get_name(city_b)
...
def add_trees(t1, t2):
    ...
    if is_leaf(t1) or is_leaf(t2):
        ...
        length = min(len(branches(t1)),len(branches(t2)))
        return tree(label(t1) + label(t2),
                    [add_trees(branches(t1)[i], branches(t2)[i])
                     for i in range(length)]
                    + branches(t1)[length:] + branches(t2)[length:])
