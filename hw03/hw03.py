...
def quadratic(x, a, b, c):
    ...
    def f(t):
        return a*t*t + b*t + c
    extreme_point = -b / (2 * a)
    if extreme_point > upper_bound(x) or extreme_point < lower_bound(x):
        up_value = max(f(lower_bound(x)), f(upper_bound(x)))
        low_value = min(f(lower_bound(x)), f(upper_bound(x)))
    else:
        up_value = max(f(lower_bound(x)), f(upper_bound(x)), f(extreme_point))
        low_value = min(f(lower_bound(x)), f(upper_bound(x)), f(extreme_point))
    return interval(low_value, up_value)
