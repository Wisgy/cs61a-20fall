...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        to_del_card = []
        player.deck.cards = [Card(card.name, card.attack+other_card.attack, card.defense+other_card.defense) for card in player.deck.cards]
        for card in opponent.deck.cards:
            if card.attack == other_card.attack or card.defense == other_card.defense:
                to_del_card.append(card)
        for card in to_del_card:
